@extends('Layout')
@section('Dashboard')
    <h1 class="font-bold text-4xl text-center my-10">Data Bahan</h1>
    ...
    <div class="overflow-x-auto mb-10">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <form action="" method="POST">
                        @csrf
                        <td>
                            <input name="name" type="text" placeholder="Nama Komponen"
                ...
                @foreach ($bahan as $item)
                    @if (Request::url() === route('aset.bahan.edit', ['id' => $item->id]))
                        <tr>
                            <td>

                            </td>

                            <form class=" w-full grid" action="{{ route('aset.bahan.edit', ['id' => $item->id]) }}"
                                method="POST">
                                @csrf
                                <td>
                                    <input name="name" value="{{ $item->name }}" type="text"
                                        placeholder="Nama Komponen" class="input input-bordered w-full max-w-md" />
                                </td>
                                <td>
                                    <input name="harga" value="{{ $item->harga }}" type="text" placeholder="harga"
                                        class="input input-bordered w-full max-w-md" />
                                </td>
                                <td>
                                    <input name="jumlah" value="{{ $item->jumlah }}" type="text" placeholder="Jumlah"
                                        class="input input-bordered w-full max-w-md" />
                                </td>
                                <td class=" flex justify-center items-center">
                        ...
                        <tr>
                            ...
                            <td>
                                <div class=" flex justify-center space-x-2">
                                    <a href="{{ route('aset.bahan.edit', ['id' => $item->id]) . '?page=' . request()->page . '&pagination=' . $pagination . '&search=' . $search }}"
                                    ...
                                    <form action="{{ route('aset.bahan.delete', ['id' => $item->id]) }}" method="post">
                                        @csrf
                                        <button type="submit"
        ...
        <div class=" text-xs flex w-full pl-3  pt-3 border-t ">
            <p>Halaman {{ $bahan->currentPage() }} Dari {{ $bahan->lastPage() }} </p>
            <div class="ml-auto flex text-lg font-bold">

                <a
                    @if ($bahan->previousPageUrl()) class="mr-10 " href="{{ $bahan->previousPageUrl() . '&pagination=' . $pagination }}"
                @else class="mr-10 text-slate-300" @endif>
                    < </a>

                        <a
                            @if ($bahan->nextPageUrl()) class="mr-10 " href="{{ $bahan->nextPageUrl() . '&pagination=' . $pagination }}" @else class="mr-10 text-slate-300" @endif>>
                        </a>
            </div>
        </div>
    </div>
