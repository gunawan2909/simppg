@extends('Layout')
@section('Dashboard')
    <h1 class="font-bold text-4xl text-center my-10">Data Komponen ME</h1>
    ...
    <div class="overflow-x-auto mb-10">
        <table class="table ">
            ...
            <tbody>
                ...
                @foreach ($komponen as $item)
                    @if (Request::url() === route('aset.komponen.edit', ['id' => $item->id]))
                        <tr>
                            <td>

                            </td>

                            <form class=" w-full grid" action="{{ route('aset.komponen.edit', ['id' => $item->id]) }}"
                                method="POST">
                                @csrf
                                <td>
                                ...
                                <td>
                                    <input name="lokasi" value="{{ $item->lokasi }}" type="text" placeholder="Lokasi"
                                        class="input input-bordered w-full max-w-md" />
                                </td>
                                <td class=" flex justify-center items-center">
                        ...
                        <tr>
                            ...
                            <td>
                                <div class=" flex justify-center space-x-2">
                                    <a href="{{ route('aset.komponen.edit', ['id' => $item->id]) . '?page=' . request()->page . '&pagination=' . $pagination . '&search=' . $search }}"
                                    ...
                                    <form action="{{ route('aset.komponen.delete', ['id' => $item->id]) }}" method="post">
                                        @csrf
                                        <button type="submit"
        ...
        <div class=" text-xs flex w-full pl-3  pt-3 border-t ">
            <p>Halaman {{ $komponen->currentPage() }} Dari {{ $komponen->lastPage() }} </p>
            <div class="ml-auto flex text-lg font-bold">

                <a
                    @if ($komponen->previousPageUrl()) class="mr-10 " href="{{ $komponen->previousPageUrl() . '&pagination=' . $pagination }}"
                @else class="mr-10 text-slate-300" @endif>
                    < </a>

                        <a
                            @if ($komponen->nextPageUrl()) class="mr-10 " href="{{ $komponen->nextPageUrl() . '&pagination=' . $pagination }}" @else class="mr-10 text-slate-300" @endif>>
                        </a>
            </div>
        </div>
    </div>
